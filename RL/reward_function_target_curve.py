...
def dense_reward(pred: str, truth: str,
    ...
    rewards = []
    truth_done = set()          # original truth indices
    t_idx = 0                   # original truth position

    for p, t in zip(align_pred, align_truth):
        if t == '-':            # insertion
            rewards.append(gap)
            continue
        # original truth position
...
class AlignTool:
    """Unified global/local alignment entry; parameters configured here."""
...
class BatchCurveTracker:
    def __init__(self):
        self.batch_count = 0

        # Define target curve parameters
        self.curves = {
            'format': {'initial': 1.5, 'decay': 0.05, 'weight': 1.0},
            'loop': {'initial': 2, 'decay': 0.03, 'weight': 1.0},
            'short': {'initial': 0.9, 'decay': 0.02, 'weight': 1.0},
            'copy_num': {'initial': 3.0, 'decay': 0.03, 'weight': 1.0},
            'edit_dis': {'initial': 0.8, 'decay': 0.02, 'weight': 1.0}
        }

    def expected_penalty(self, objective_name):
        """Compute expected penalty for the current batch."""
        params = self.curves[objective_name]
        return params['initial'] * np.exp(-params['decay'] * self.batch_count)

    def __call__(self, data_sources, solution_strs, ground_truths, extra_infos):
        """
        Batch-level curve-tracking reward function

        Args:
            data_sources: List of data source names
            solution_strs: List of decoded response strings
        ...
        # First, check if scoring on the validation dataset
        is_validation = any("val" in extra_info.get("split", "") for extra_info in extra_infos)

        # Only update batch_count and compute expected values during training
        if not is_validation:
            self.batch_count += 1
            expected_penalties = {
                obj_name: self.expected_penalty(obj_name)
                for obj_name in self.curves.keys()
            }
        else:
            # Do not use curve tracking during validation
            expected_penalties = None
        ...
        for data_source, solution_str, ground_truth, extra_info in zip(
            ...
            # 1) format penalty
            pred_clean, illegals = sanitize_and_penalize(sol_str)  #
            fmt_pen = 1 * illegals
            print('fmt_pen :', fmt_pen)

            # 2) loop penalty
            loop_len, loop = loop_penalty_anystart(pred_clean, min_unit=200)  #
            loop_pen = 1 * loop_len / len(ground_truth)  #
            print('loop_pen :', loop_pen)

            # 3) short penalty
            short_cnt = detect_repeat_shortcut(pred_clean, ground_truth, gap_min=40)
            short_pen = 1 * short_cnt
            print('short_pen :', short_pen)

            # 4) copy num distance penalty
            copy_num_err = copy_num_error_slide(pred_clean, ground_truth, frag_len=15, frags_per_kb=1000)
            copy_num_pen = 1 * copy_num_err
            print('copy_num_pen :', copy_num_pen)

            # 5) edit distance penalty
            similarity = AlignTool.similarity(pred_clean, ground_truth)  # range is 0-1
            ed_pen = 1 * (1 - similarity)  #
            print('ed_pen :', ed_pen)
            ...
            if sol_str == ground_truth.upper():
                ...
                cov_r = 1 * coverage_f1_kmergt(pred_clean, extra_info['dna_sequence'], ground_truth, block=50,
                                           min_cov_ratio=0.75,
                                           max_align_ratio=1.1, beta=1.0)
            print('basic_reward :', cov_r)  #

            # Base reward
            base_reward = cov_r

            # Compute actual penalties
            actual_penalties = {
                'format': fmt_pen,
                'loop': loop_pen,
                'short': short_pen,
                'copy_num': copy_num_pen,
                'edit_dis': ed_pen
            }

            if not is_validation:
                # Add tracking_bonus only during training (batch-level expectations)
                tracking_bonus = 0.0
                for obj_name, actual_penalty in actual_penalties.items():
                    expected = expected_penalties[obj_name]
                    weight = self.curves[obj_name]['weight']
                    # Drops faster than expected -> positive reward
                    tracking_bonus += (expected - actual_penalty) * weight
                final_reward = base_reward + tracking_bonus
            else:
                # During validation, use the equal method for penalties + base reward
                final_reward = base_reward - fmt_pen - loop_pen - short_pen - copy_num_pen - ed_pen

            rewards.append(final_reward)

        return rewards

    # Export as compute_score_batch


compute_score_batch = BatchCurveTracker()

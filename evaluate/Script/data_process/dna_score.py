...
class RewardMetrics:
    ...
    loop_penalty: float
    short_penalty: float
    copy_num_penalty: float
    edit_dist_penalty: float

    def __str__(self):
        return (f"RewardMetrics:\n"
                f"  Total Score: {self.total_score:.4f}\n"
                f"  --------------------------\n"
                f"  Coverage:    {self.coverage_score:.4f}\n"
                f"  Format Pen:  {self.format_penalty:.4f}\n"
                f"  Loop Pen:    {self.loop_penalty:.4f}\n"
                f"  Short Pen:   {self.short_penalty:.4f}\n"
                f"  CopyNum Pen: {self.copy_num_penalty:.4f}\n"
                f"  EditDist Pen:{self.edit_dist_penalty:.4f}")
...
# ---------- Previous direct similarity calculation ----------
...
# ---------- Previous SM-based prefix match (may be unsuitable) ----------
...
def reward_function_equal(pred_seq_str: str, gt_seq_str: str, reads_list: list[str]) -> float:
   ...
   # total_score = cov_r + fmt_pen + loop_pen + short_pen + copy_num_pen + ed_pen

    # 7) Build and return struct
    metrics = RewardMetrics(
        ...
        copy_num_penalty=copy_num_pen,
        edit_dist_penalty=ed_pen
    )
    
    return metrics


def coverage_f1_nogt(pred: str, reads: list[str],
                       block: int = 80,
                       min_cov_ratio: float = 0.8,
                       min_identity: float = 0.8,
                       max_align_ratio: float = 1.1,
